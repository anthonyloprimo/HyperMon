...
(function (g) {
    ...
    async function startBgm(opts) {
        const { 
            ...
            fade = DEFAULT_BGM_FADE, 
            fadeInMs = FADE_IN_MS, 
            loop = DEFAULT_BGM_LOOP, 
            once = false, 
            returnTo = null 
        } = opts;
        const key = id || explicitUrl;
        if (!key) return;
        const url = explicitUrl || BGM_MANIFEST[key] || key; // support manifest ids or raw paths
        if (!url) return;
    ...
    async function stopBgmInternal({ fade = DEFAULT_BGM_FADE, fadeMs = FADE_OUT_MS } = {}) {
        if (!currentBgm) return;
        const { media, gain } = currentBgm;
        if (fade) {
            const dur = Number.isFinite(fadeMs) ? Math.max(0, fadeMs) : FADE_OUT_MS;
            if (gain) fadeTo(gain, 0.0, dur);
            else fadeElementVolume(media, 0.0, dur);
