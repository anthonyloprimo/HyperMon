(function (g) {
    "use strict";

    const TILE = 16;

    const Collision = {
        attachMap,
        squareAt,
        canStartStep,
        setIgnoreSolids,
        isIgnoringSolids,
        // helpers (exported for later phases / debugging)
        toTileX: px => (px / TILE) | 0,
        toTileY: py => (py / TILE) | 0,
        step
    };
    g.Collision = Collision;

    // current map views
    let SQUARES = null;
    let GRID = null;
    let VOID_ID = 0;
    let IGNORE_SOLIDS = false; // toggled by debug panel to let player bypass walls

    function setIgnoreSolids(flag) {
        IGNORE_SOLIDS = !!flag;
    }

    function isIgnoringSolids() {
        return IGNORE_SOLIDS;
    }

    function attachMap(map) {
        // squares - tile definitions w/ collision/attributes
        SQUARES = map && map.squares || null;

        // grid - supports map.grid.squares or map.map
        if (map && map.grid && map.grid.squares) {
            GRID = map.grid.squares;
        } else if (map && map.map) {
            GRID = map.map;
        } else {
            GRID = null;
        }

        // void square ID
        VOID_ID = (map && (map.voidSquare ?? map.map?.voidSquare)) ?? 0;
    }

    // function squareAt(tx, ty) {  // old method for single-map situations
    //     if (!GRID || !SQUARES) return null;
    //     if (tx < 0 || ty < 0 || tx >= GRID.width || ty >= GRID.height) return null;
    //     const sid = GRID.ids[ty * GRID.width + tx];
    //     const idx = (sid == null ? VOID_ID : sid);
    //     return SQUARES[idx] || null;
    // }
    function squareAt(tx, ty) {
        if (!GRID || !SQUARES) return null;

        // In-bounds on current map
        if (tx >= 0 && ty >= 0 && tx < GRID.width && ty < GRID.height) {
            const sid = GRID.ids[ty * GRID.width + tx];
            const idx = (sid == null ? VOID_ID : sid);
            return SQUARES[idx] || null;
        }

        // New code: out-of-bounds; consult World to resolve a neighbor tile
        if (window.World && typeof World.worldSquareAt === "function") {
            const sq = World.worldSquareAt(tx, ty);
            if (sq) return sq;
            // If no owning neighbor, fall through to null -> treated as blocked for movement.
        }

        return null;
    }

    // dir: "up"|"down"|"left"|"right" OR "N"|"S"|"W"|"E"
    function step(tx, ty, dir) {
        switch (dir) {
            case "up": case "N":    return [tx,     ty - 1];
            case "down": case "S":  return [tx,     ty + 1];
            case "left": case "W":  return [tx - 1, ty    ];
            case "right": case "E": return [tx + 1, ty    ];
            default:                return [tx,     ty    ];
        }
    }

    // Check if a square is blocked for a given movement mode.
    // moveMode: "WALK" | "BIKE" | "SURF"
    // Notes:
    //  - SOLID is always blocked regardless of mode.
    //  - Water is blocked for WALK/BIKE but allowed for SURF.
    function isBlockedForMode(sq, moveMode) {
        if (!sq) return true; // treat missing as blocked
        if (IGNORE_SOLIDS) return false;
        const mode = String(moveMode || 'WALK').toUpperCase();
        const c = sq.collision || {};

        // Hard solids always block
        if (c.solid) return true;

        // Water blocks normal movement, unless surfing
        if (c.surface === 'water' && mode !== 'SURF') return true;

        // Otherwise walkable
        return false;
    }
    // Check square flags (old method)
    // function isSolidFromCollision(sq) {
    //     if (!sq) return true;
    //     const c = sq.collision || {};
    //     return !!c.solid;
    // }
    function dirToCard(dir) {
        if (dir == null) return null;
        const d = String(dir).trim().toUpperCase();

        if (d === "UP" || d === "NORTH") return "N";
        if (d === "RIGHT" || d === "EAST") return "E";
        if (d === "DOWN" || d === "SOUTH") return "S";
        if (d === "LEFT" || d === "WEST") return "W";

        if ("NESW".includes(d)) return d;
        return d[0] ?? null;
    }
    function canLedgeHopFrom(tx, ty, dir, moveMode) {
        const want = dirToCard(dir);
        const [nx, ny] = step(tx, ty, dir);
        const edgeSq = squareAt(nx, ny);
        if (!edgeSq) return null;

        const c = edgeSq.collision || {};
        const edgeLedge = c.ledge || null;
        if (edgeLedge !== want) return null;

        const [lx, ly]  = step(nx, ny, dir);
        const landingSq = squareAt(lx, ly);
        // Use mode-aware blocking so ledge hops won't land on forbidden tiles (e.g., water while walking)
        if (!landingSq || isBlockedForMode(landingSq, moveMode)) return null;

        return { via: [nx, ny], landing: [lx, ly] };
    }
    // solid/out-of-bounds movement restriction
    // returns: { ok:boolean, mode:"WALK"|"HOP", reason?:string, landing?:[tx,ty] }
    // opts.moveMode: "WALK" | "BIKE" | "SURF" (defaults to "WALK")
    function canStartStep(tx, ty, dir, opts) {
        const moveMode = (opts && opts.moveMode) ? String(opts.moveMode) : "WALK";
        // ledge hop check
        const hop = canLedgeHopFrom(tx, ty, dir, moveMode);
        console.debug(hop);
        if (hop) {
            console.debug("Yes, we can hop!")
            return {
                ok: true,
                mode: "HOP",
                via: hop.via,
                landing: hop.landing,
                distanceTiles: 2
            }
        }

        // Normal walking
        const [nx, ny] = step(tx, ty, dir);
        const destSq = squareAt(nx, ny);
        if (!destSq) return { ok: false, reason: "oob" };
        const destCollision = destSq.collision || {};
        if (IGNORE_SOLIDS && destCollision.solid) {
            return {
                ok: true,
                mode: "WALK",
                landing: [nx, ny],
                distanceTiles: 1,
                ignoredSolid: true
            };
        }
        if (isBlockedForMode(destSq, moveMode)) return { ok: false, reason: "solid" };
        return { ok: true, mode: "WALK", landing: [nx, ny], distanceTiles: 1 };
    }
})(window);
